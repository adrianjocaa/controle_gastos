...
// A LÓGICA ABAIXO FOI ATUALIZADA PARA ENVIAR DADOS DE PARCELAMENTO
gastoForm.addEventListener('submit', async (e) => {
    ...
    const quantidade_parcelas = parseInt(document.getElementById('quantidade_parcelas').value);
    ...
    if (parcelado && (isNaN(quantidade_parcelas) || quantidade_parcelas < 2)) {
        showMessage('Selecione "Parcelado" e insira um número de parcelas maior que 1.', 'error');
        return;
    }
    ...
    const payload = {
        ...
        vencimento,
        parcelado,
        quantidade_parcelas: parcelado ? quantidade_parcelas : null
    };
    ...
    try {
        ...
        if (response.ok) {
            showMessage('Gasto(s) adicionado(s) com sucesso!', 'success');
            gastoForm.reset();
            document.getElementById('campo-parcelas').style.display = 'none';
            document.getElementById('valor').placeholder = 'Valor (R$)';
